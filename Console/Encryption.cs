...
namespace Shell {
	public class Encryption {
		...
		public bool readKey(string keyfile) {

			try {
				var textblock = decodeBase64(File.ReadAllText(keyfile));
				var guid = textblock.Split('.')[2];

				comm_guid = guid;
				comm_iv = textblock.Split('.')[0];
				comm_key = textblock.Split('.')[1];

				Console.WriteLine(textblock);

				return true;
			} catch (Exception ex) {
				Console.WriteLine("Error trying to read Key: {0}", ex);
			}
		...
		public bool encrypt(string block) {
			...
			try {
				...
				File.WriteAllText(String.Format(@"{0}\SSI-{1}.txt", defaultFolder, comm_guid), Convert.ToBase64String(encrypted));
				return true;
			}
			catch (Exception ex) {
